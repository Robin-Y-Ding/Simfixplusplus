...
def checkout_buggy_file(csv_info):
	...
	for row in csv_info:
		...
		path2 = proj_home_path + "/" + proj_name.lower() + "/" + proj_name.lower() + "_" + bug_id \
				+ "_" + "buggy"
		checkout_command = "defects4j checkout -p " + proj_name + " -v " + bug_id + "b" + " -w " + path2
...
def generate_one_line_bug_file(csv_info):
	file_output = open("./one_line_bug.txt", "w+")
	proj_home_path = "/home/robin/Documents/Simfixplusplus/simfix-automatic-build/d4j/projects"
	for row in csv_info:
		proj_name = row[0]
		bug_id = row[1]
		...
		buggy_file_path = path2 + "/" + detail_path
		
		file_input=open(buggy_file_path)
...
#checkout_buggy_file(csv_info)
generate_one_line_bug_file(csv_info)
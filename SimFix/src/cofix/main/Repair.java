	...
	public Status fix(Timer timer, String logFile, int currentTry) throws IOException{
		...
		for(Pair<String, Integer> loc : locations){
			...
			for(CodeBlock oneBuggyBlock : buggyBlockList){
				...
				// newly added
				System.out.println("\n--current buggy code--\n" + "BUGGY LINE:" + range.getFirst() + " to " + range.getSecond() + "\n" + "Codes:\n" + oneBuggyBlock.toSrcString().toString() + "---end of code--\n");
				...
				// Separation point for simfix and simfix++
				if(Constant.PROJ_TECH.equals("patch")){
					System.out.println("project_tech: patch");
				}

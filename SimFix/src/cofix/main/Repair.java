...
//		}
	...
	public Status fix(Timer timer, String logFile, int currentTry) throws IOException{
		...
		for(Pair<String, Integer> loc : locations){
			...
			//Robin: file is the file name and loc.getSecond() is the thing we want
			CodeBlock buggyblock = BuggyCode.getBuggyCodeBlock(file, loc.getSecond());
			// Robin: buggyblock is already AST nodes. Contain 10 lines of code including the buggy line
			...
			for(CodeBlock oneBuggyBlock : buggyBlockList){
				...
				//Robin: now we get the buggy code string
				...
				//Robin: CodeBlock is the candidate code and double is the similarity score
...
//					System.out.println(similar.getFirst().toSrcString().toString());
					...
					//Robin: one example of modification: [[REV | ReturnStatement | 0]return NaN;=>INF]]
...
//		System.out.println(stringBuffer.toString());
	...
	private List<Set<Integer>> consistentModification(List<Modification> modifications){
		//check this method in all-version
